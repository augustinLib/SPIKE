...
def evaluate_dataset(dataset_name: str, 
    ...
    with open(f"/mnt/data1/SPIKE/data/embedding_instruction/{model_name}/{args.dataset}.json", "r") as f:
    ...
    max_seq_length = MODEL_MAX_LEN_DICT[args.model]
    ...
    for _, row in tqdm(query_df.iterrows()):            
        ...
        if len(tokenized_query) > max_seq_length:
            tokenized_query = tokenized_query[:max_seq_length]
            instructed_query = tokenizer.convert_tokens_to_string(tokenized_query)

        query_embedding = model.encode(instructed_query, show_progress_bar=False)
...
if __name__ == "__main__":
    ...
    model = GritLM(args.model, mode="embedding", torch_dtype="auto")
